...
// 地形网格二维拼成一维的长度
// 在C++中，一维数组在内存中是连续分配的，而二维数组实际上是"数组的数组"，可能导致内存分散，特别是在动态分配时。
// 连续内存分配通常有更好的缓存性能。
...
int main(int argc, char **argv) {
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      // 滚动数组，更新地形网格数据
      // 更新Y列
      // 如果载体的坐标与地形网格中心坐标的反投影坐标不在一个体素大小范围内
      ...
      for (int i = 0; i < laserCloudCropSize; i++) {
        ...
        // 以载体为中心，建立一个正方形的地形网格
        // 计算点云在地形网格中的索引
        // +terrainVoxelSize/2是为了四舍五入
        // 地形网格中心是(terrainVoxelHalfWidth,terrainVoxelHalfWidth)
        int indX =
        ...
        if (indX >= 0 && indX < terrainVoxelWidth && indY >= 0 &&
          ...
          terrainVoxelUpdateNum[terrainVoxelWidth * indX + indY]++; // 更新格子中的点数量
        }
      }
            ...
            for (int dY = -10; dY <= 10; dY++) {
              if (indX + dX >= 0 && indX + dX < planarVoxelWidth &&
                ...
                if (planarVoxelConn[ind] == 0 &&
                  ...
                  // 如果当前的平面格子的高度值和周边的平面格子的高度值的差的绝对值
                  // 小于阈值
                  // 说明当前的平面格子和周边的平面格子是连接的
      ...
      for (int i = 0; i < terrainCloudSize; i++) {
        ...
        if (point.z - vehicleZ > lowerBoundZ - disRatioZ * dis &&
          ...
          if (indX >= 0 && indX < planarVoxelWidth && indY >= 0 &&
            ...
            // 绝对值，那么在vehicleHeight外的点就会被过滤掉
            // 检查过地形连接性就需要地形是连接的，要么就没有检查过连接性
      ...
      // 当局部地图点云的点距离载体的平面距离小于localTerrainMapRadius时
      // 将点云的点加入到terrainCloudElev中
