...
void laserCloudHandler(
  ...
  for (int i = 0; i < laserCloudSize; i++) {
    ...
    // 点云的点在水平面上距离载体的距离，在同一个系下的
    float dis = sqrt((pointX - vehicleX) * (pointX - vehicleX) +
                     (pointY - vehicleY) * (pointY - vehicleY));
    // 点云的点到载体的z轴距离在设定范围内
    // minRelZ-disRatioZ*dis ~ maxRelZ+disRatioZ*dis
    // 且点到载体的xy平面距离在设定范围内
...
int main(int argc, char **argv) {
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      // 从体素网格中提取车辆周围一定范围内的点云数据，并合并到一个单一的点云中，为后续的地形分析做准备
      terrainCloud->clear();
      // 遍历以车辆为中心的11×11的体素区域（从中心向外扩展5个体素）
      ...
      for (int i = 0; i < terrainCloudSize; i++) {
        point = terrainCloud->points[i];
        // 检查点的相对高度是否在设定的范围内（minRelZ到maxRelZ）
        if (point.z - vehicleZ > minRelZ && point.z - vehicleZ < maxRelZ) {
          int indX =
              static_cast<int>((point.x - vehicleX + planarVoxelSize / 2) /
                               planarVoxelSize) +
          ...
          if (indX >= 0 && indX < planarVoxelWidth && indY >= 0 &&
              indY < planarVoxelWidth) {
            // 动态障碍物点数少于阈值minDyObsPointNum或者没有启用动态障碍物清除
            if (planarVoxelDyObs[planarVoxelWidth * indX + indY] <
              ...
              // 启用了considerDrop选项
              // 不仅考虑高于地面的点，也考虑低于地面的点（如悬崖或坑洞）
              if (considerDrop)
                disZ = fabs(disZ);
              int planarPointElevSize =
                  planarPointElev[planarVoxelWidth * indX + indY].size();
              // 高程值必须是非负的（或者如果考虑落差，则已经取了绝对值）
              // 高程值必须小于车辆高度（避免考虑太高的点）
              // 该体素中必须有足够多的高程样本点
