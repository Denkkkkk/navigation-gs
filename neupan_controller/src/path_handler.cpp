...
: transform_tolerance_(transform_tolerance), tf_(tf), costmap_ros_(costmap_ros),
  logger_(rclcpp::get_logger("PathHandler"))
{
}
...
nav_msgs::msg::Path PathHandler::getRelevantGlobalPlan(
  const geometry_msgs::msg::PoseStamped & pose,
  double max_robot_pose_search_dist,
  bool reject_unit_path)
{
  if (global_plan_.poses.empty()) {
    throw InvalidPath("Received plan with zero length");
  }

  if (reject_unit_path && global_plan_.poses.size() == 1) {
    throw InvalidPath("Received plan with length of one");
  }

  // 将机器人位姿转换到全局路径坐标系（保持一致性）
  geometry_msgs::msg::PoseStamped robot_pose;
  if (!transformPose(global_plan_.header.frame_id, pose, robot_pose)) {
    throw ControllerTFError("Unable to transform robot pose into global plan's frame");
  }
  ...
  if (global_plan_.poses.begin() != closest_pose_upper_bound && global_plan_.poses.size() > 1 &&
    selection_begin == std::prev(closest_pose_upper_bound))
  {
    selection_begin = std::prev(std::prev(closest_pose_upper_bound));
  }

  // 确定路径段终点（基于距离或代价地图范围）
  const double max_costmap_extent = getCostmapMaxExtent();
  auto selection_end = std::find_if(
    selection_begin, global_plan_.poses.end(),
    [&](const auto & global_plan_pose) {
      return euclidean_distance(global_plan_pose, robot_pose) > max_costmap_extent;
    });

  // 直接复制路径段，不进行坐标变换
  nav_msgs::msg::Path selected_plan;
  selected_plan.header = global_plan_.header;  // 保持原始坐标系
  selected_plan.poses.assign(selection_begin, selection_end);

  // 路径裁剪（移除已经经过的部分）
  global_plan_.poses.erase(begin(global_plan_.poses), selection_begin);

  if (selected_plan.poses.empty()) {
    throw InvalidPath("Resulting plan has 0 poses in it.");
  }
  ...
  bool reject_unit_path)
{
  ...
  auto transformGlobalPoseToLocal = [&](const auto & global_plan_pose) {
      ...
      if (!transformPose(costmap_ros_->getBaseFrameID(), stamped_pose, transformed_pose)) {
        throw ControllerTFError("Unable to transform plan pose into local frame");
      }
